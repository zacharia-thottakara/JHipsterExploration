...
import edu.jhipsterex.example.config.KafkaSseConsumer;
import edu.jhipsterex.example.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/j-hipster-ex-kafka")
public class JHipsterExKafkaResource {

    private final Logger log = LoggerFactory.getLogger(JHipsterExKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public JHipsterExKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
