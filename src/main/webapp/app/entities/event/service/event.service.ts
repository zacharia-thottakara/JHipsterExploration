...
type RestOf<T extends IEvent | NewEvent> = Omit<T, 'initial'> & {
  initial?: string | null;
};
...
export class EventService {
  ...
  protected convertDateFromClient<T extends IEvent | NewEvent | PartialUpdateEvent>(event: T): RestOf<T> {
    return {
      ...event,
      initial: event.initial?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEvent: RestEvent): IEvent {
    return {
      ...restEvent,
      initial: restEvent.initial ? dayjs(restEvent.initial) : undefined,
    };
  }
